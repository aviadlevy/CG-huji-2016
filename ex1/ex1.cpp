...
int main(int argc, char* argv[])
{
    ...
    glutTimerFunc(10, timer, 0);   // uint millis int value
...
void mouse(int button, int state, int x, int y)
{
    // create ball when left button clicked
    if(button == GLUT_LEFT_BUTTON && state == GLUT_DOWN)
    {
		_model.createBall(glm::vec2(x / _model.getWidth() * 2 - 1, -(y / _model.getHeight() * 2 - 1)));
    }
...
void timer(int value) {
    ...
    glutTimerFunc(15, timer, ++value);   // uint millis int value
