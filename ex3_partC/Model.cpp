...
_vaoCircle(0), _vboCircle(0), _vaoModel(0), _vboModel(0), _vaoModelFaces(0), _vboModelFace(0), _elementBo(0),
			...
			_textureMode(TEXTURE_MARBEL), _spec(200), _textureScale(TEXTURE_SCALE), _turbMag(TURB_MAGN),
_sphereFile("textures/spheremap2.bmp"), _brickFile("textures/brickwork-texture.bmp"), _brickBumpFile("textures/brickwork-bump-map.bmp")
{
...
void Model::init(char* filePath)
{
	...
	_textureScaleUV = glGetUniformLocation(_program, "textScale");
	_textureModeUV = glGetUniformLocation(_program, "textMode");
	_turbMagnUV = glGetUniformLocation(_program, "turbMagn");
	...
	loadTexture(_sphereFile, &_sphereText);
	loadTexture(_brickFile, &_brickText);
	loadTexture(_brickBumpFile, &_brickBumpText);
...
void Model::draw3dModel()
{
	...
	glUniform1f(_turbMagnUV, _turbMag);
	...
	if(_textureMode == TEXTURE_MIRROR || _textureMode == TEXTURE_BRICK)
	{
		...
		if(_textureMode == TEXTURE_MIRROR)
		{
			...
			glBindTexture(GL_TEXTURE_2D, _sphereText);
		}
		if(_textureMode == TEXTURE_BRICK)
		{

			glActiveTexture(GL_TEXTURE0);
			glBindTexture(GL_TEXTURE_2D, _brickText);
			int loc1 = glGetUniformLocation(_program, "my_specmap");
			glUniform1i(loc1, 1);
			glActiveTexture(GL_TEXTURE1);
			glBindTexture(GL_TEXTURE_2D, _brickBumpText);
		}
	}
...
//	_isPrespective = true;
	...
	_spec = 200;
	_textureMode = TEXTURE_MARBEL;
	_textureScale = TEXTURE_SCALE;
	_turbMag = TURB_MAGN;
}
...
void Model::loadTexture(const char *fileName, GLuint *texture) {
	BImage image(fileName);

	glGenTextures(1, texture); // Generate a texture in OpenGL
	glBindTexture(GL_TEXTURE_2D, *texture); // Bind texture before setting its properties
...
void Model::decreaseScaleTexture() {
	_textureScale--;
	if(_textureScale < 1)
		_textureScale = 1;
}

void Model::increaseScaleTexture() {
	// we didn't find reason to bound this
	_textureScale++;
}

void Model::decreaseTurb() {
	_turbMag--;
	if(_turbMag < 0)
		_turbMag = 0;
}

void Model::increaseTurb() {
	// we didn't find reason to bound this
	_turbMag++;
}
