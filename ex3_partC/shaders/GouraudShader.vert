#version 330

layout(location = 0) in vec4 positionModel;
layout(location = 1) in vec3 normalModel;

uniform mat4 modelTransform;

uniform float specExp;
uniform vec3 ambientColor = vec3(1.0, 1.0, 1.0);
uniform vec3 lightColor1 = vec3(1.0, 0.9, 0.7);
uniform vec3 lightColor2 = vec3(0.6, 0.6, 1.0);

uniform vec3 light_position1 = vec3(3.0, 2.0, 1.0);
uniform vec3 light_position2 = vec3( -3.0, 0.0,  1.0);

uniform vec3 ka = vec3(0.1, 0.1, 0.1);
uniform vec3 kd = vec3(0.3, 0.3, 0.3);
uniform vec3 ks = vec3(0.3, 0.3, 0.3);

out vec4 inColor;

void main()
{
    gl_Position  = modelTransform * positionModel;
    
    
    vec3 nTemp = normalize((modelTransform * vec4(normalModel, 0.0)).xyz);
    
    vec3 shadingPosition = (modelTransform * positionModel).xyz;
    
    vec3 light1 = normalize(light_position1 - shadingPosition);
    vec3 light2 = normalize(light_position2 - shadingPosition);
    vec3 view = normalize(vec3(0.0, 0.0, 0.0) - shadingPosition);
    
    vec3 diffColor = ka * ambientColor + lightColor1 * kd * max(dot(nTemp, light1), 0.0)  + lightColor2 * kd *
                    max(dot(nTemp, light2), 0.0);
    
    
    vec3 reflect1 = normalize(reflect(-light1, nTemp));
    
    vec3 reflect2 = normalize(reflect(-light2, nTemp));
    
    vec3 specularColor = ks * pow(max(dot(view, reflect1), 0.0), specExp) * lightColor1  + ks *
                    pow(max(dot(view, reflect2), 0.0), specExp) * lightColor2;
    
    inColor = vec4(diffColor + specularColor, 1.0);
}
